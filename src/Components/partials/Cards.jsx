    ...
    <div className='flex flex-wrap w-full px-[5%] bg-[#1F1E24]'>
      {data.map((cards, index) => (
        <Link to={`/${cards.media_type || title }/details/${cards.id}`} className='relative w-[25vh] mr-[5%] mb-[5%] mt-[30px]' key={index}>
          <img className='shadow-[8px_17px_38px_2px_rgba(0,0,0,.5)] h-[40vh] object-cover rounded-xl' src={`https://image.tmdb.org/t/p/original/${cards.poster_path || cards.backdrop_path || cards.profile_path}`} alt="" />
